...
class DQNAgent:
    ...
    MODEL_FILE = "models/best/DQN_model____74.80max____1.02 avg___-4.60min.h5"
    ...
    def __init__(self, mode="train", nodes=32, memory_size=500, final_act="linear", minibatch=32, lr=1e-3,
        ...
        if mode == "train":
            ...
            self.target_model = tf.keras.models.load_model(self.MODEL_FILE)
            self.target_model.set_weights(self.model.get_weights())
...
def learn(nodes=32, num_episodes=2000, memory_size=5000, final_act="linear", minibatch=32, lr=1e-3):
