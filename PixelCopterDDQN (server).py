...
class DDQNAgent:
    ...
    def __init__(self, mode="train", nodes=32, num_episodes=1000, memory_size=500, final_act="linear", minibatch=32,
        ...
        self.MODEL_NAME = f"({lr} {minibatch} {memory_size} {nodes} {final_act} {num_episodes})"
...
def main(nodes=32, num_episodes=1000, memory_size=500, final_act="linear", minibatch=32, lr=1e-3):
    ...
    for episode in tqdm(range(1, num_episodes + 1)):
        ...
        can_update = episode_rewards[-1] == np.max(episode_rewards)
        if can_update or episode == 1:
            ...
            agent.model.save(
                f'models/{agent.MODEL_NAME}/{agent.MODEL_NAME}_{max_reward:_>7.2f}max_{average_reward:_>7.2f}avg_{min_reward:_>7.2f}min_model.h5')
        # Decay epsilon
...
def init():
    ...
    hidden_layer_nodes = np.arange(32, 241, 64)  # 32, 64, 96, 128 ....
    num_episodes = [1000]
    replay_memory_size = [3000, 5000]
